    ...
    // =====================================================================
    // Exam data store
    // courseExamsMap: Map<courseName, { exams: [{type, date, time}], courseCode? }>
    // Populated from catalog course objects whenever one is fetched or displayed,
    // and also manually editable. Persisted to localStorage independently of
    // rawCourses so it survives semester switches and clear-all.
    // =====================================================================
    const COURSE_EXAMS_STORAGE_KEY = 'courseExamsData';
    let courseExamsMap = new Map(); // courseName → { exams: [{type,date,time}], courseCode }
    ...
    function registerCourseExams(course) {
        ...
        // An existing entry with a non-empty exam list is user-owned data —
        // never overwrite it (the user may have edited it manually).
        // An existing entry with an EMPTY exam list is a stale seed from an
        // older version of the app that didn't yet fetch exam times; treat it
        // the same as "nothing stored yet" and replace it with fresh data.
        const isStaleEmptySeed = existing && existing.exams && existing.exams.length === 0 &&
        ...
        if (!existing || isStaleEmptySeed) {
            ...
            // Entry already exists with real exam data (user may have edited it)
            // — only update the metadata fields, never overwrite the exam list.
            let changed = false;
    ...
    function renderSearchAddDialog(course) {
        ...
        const shoamLinkHtml = course.shoamId
            ? ` · <a href="https://courses.biu.ac.il/CourseDetails.aspx?lid=${course.shoamId}" target="_blank" rel="noopener" style="color:var(--primary); text-decoration:none;" title="פתח בשוהם">שוהם ↗</a>`
            : '';
        document.getElementById('searchAddMeta').innerHTML = `${course.courseCode} · ${creditsDisplay} נ"ז${shoamLinkHtml}`;
        ...
        // Merge stored exams into the course object so renderExams uses the latest
        const stored = courseExamsMap.get(course.nameHe);
        const courseWithExams = stored && stored.exams.length > 0
            ? { ...course, exams: stored.exams }
            : course;
        ...
        renderExamsEditSection(courseWithExams);
    }
    ...
    function renderExamsEditSection(course) {
        ...
        if (!el) return;
        const stored = courseExamsMap.get(course.nameHe);
        const exams = stored ? stored.exams : (course.exams || []);
        const courseId = course.id;
        // Shoam link — prefer the live course object, fall back to what's stored in the map
        const shoamId = course.shoamId || (stored && stored.shoamId) || null;
        const shoamLinkHtml = shoamId
            ? `<a href="https://courses.biu.ac.il/CourseDetails.aspx?lid=${shoamId}" target="_blank" rel="noopener"
                  style="font-size:12px; color:var(--primary); text-decoration:none; margin-left:8px;" title="פתח בשוהם">שוהם ↗</a>`
            : '';

        let html = `<div class="exam-section exam-section-edit">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px; flex-wrap:wrap; gap:6px;">
                <div style="display:flex; align-items:center; gap:6px;">
                    <h4 style="margin:0;">ערוך מועדי בחינות</h4>${shoamLinkHtml}
                </div>
        ...
        if (!exams || exams.length === 0) {
            ...
            exams.forEach((exam, i) => {
                ...
                html += `<div class="exam-row" style="cursor:default;">
                    ...
                    <button class="icon-btn" style="font-size:13px; padding:3px 6px;"
                        onclick="openEditExam('${courseId}', '${course.nameHe.replace(/'/g,"\\'")}', ${i})"
                        title="ערוך">✏️</button>
                </div>`;
            });
        }
    ...
    /** Build the flat exam entry list for the given semester.
    ...
    function getAllExamEntries(semester) {
        ...
        courseExamsMap.forEach((data, courseName) => {
            ...
            if (!semCourseNames.has(courseName)) return; // not in this semester
     ...
     *  stored entry has an empty exam list (a stale seed from an older build
     *  of the app that didn't yet include exam times).
     *  Called both on page load (so exams are ready by the time the dialog
     *  is first opened) and again when the dialog itself opens.
     *  Silently ignores errors and re-renders the open dialog once any fetch
     *  comes back with real exam data. */
    function _seedMissingCourseExams() {
        ...
        rawCourses.forEach(c => {
            ...
            // Re-fetch if: (a) we have no entry at all, or (b) the stored
            // entry has an empty exam list — that's the stale-seed case from
            // an older version that never recorded exam times.
            const stored = courseExamsMap.get(c.name);
            const needsFetch = !stored || (stored.exams && stored.exams.length === 0);
            if (needsFetch) {
                fetchCourseDetail(courseId).then(course => {
                    if (course.exams && course.exams.length > 0 &&
                        document.getElementById('examsDialog').open) {
                        renderExamsDialog();
                    }
    ...
    function saveExamEdit() {
        ...
        courseExamsMap.set(courseName, { ...stored, exams });
        saveCourseExamsToStorage();
